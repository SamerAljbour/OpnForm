<template>
  <BaseSidebar>
    <template #mobile-header>
      <h1
        class="sm:hidden text-base font-medium text-neutral-500 tracking-wider px-2 truncate"
      >
    ...
    <template #navigation>
      ...
      >
        ...
        <NavigationList
          ...
          :loading="loading"
          :skeleton-filter="
            (item) => section.name === null && item.label !== 'Dashboard'
          "
          tracking-name="sidebar_nav_click"
          :tracking-properties="
            (item) => ({ label: item.label, form_id: form?.id })
          "
        />
      </div>
    </template>
