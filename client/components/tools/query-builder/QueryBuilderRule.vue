<template>
  <div class="flex items-center gap-2 flex-1">
    ...
    <div
      v-if="index === 0"
      class="flex-shrink-0 text-sm font-medium text-neutral-600 w-17 text-right"
    >
      {{ whereText }}
    </div>
...
const ruleDefinition = computed(() => {
  return props.config.rules.find(
...
const operatorDisplayName = computed(() => {
  const operator = props.operatorOptions.find(
    (op) => op.value === props.selectedOperator,
  );
