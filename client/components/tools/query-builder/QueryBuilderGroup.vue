<template>
  <div
    ...
    :class="[
      depth >= 1
        ? 'p-2 rounded-lg border border-gray-400/30 bg-gray-100/20'
        : '',
    ]"
  >
    <!-- Rules and Groups -->
    <div v-if="children.length > 0" class="flex flex-col gap-1">
      <template v-for="(child, index) in children" :key="index">
        <!-- Rule -->
        <div
          v-if="child.hasOwnProperty('identifier')"
        ...
        <!-- Nested Group -->
        <div
          v-else-if="child.operatorIdentifier"
          class="flex items-start gap-2"
        >
          <!-- Group prefix - same logic as rules -->
          <div
            v-if="index === 0"
            class="flex-shrink-0 text-sm font-medium text-gray-600 w-17 text-right pt-3"
          >
            {{ whereText }}
          </div>
          ...
          <div
            v-else-if="index === 1"
            class="flex-shrink-0 w-17 text-right pt-3"
          >
            <USelectMenu
    ...
    <div class="w-full">
      <UDropdownMenu :content="{ align: 'start' }" :items="addMenuItems">
        <UButton
          label="إضافة قاعدة"
...
function addRule(ruleIdentifier = null) {
  ...
  const defaultIdentifier =
    ruleIdentifier ||
    (props.config.rules.length > 0 ? props.config.rules[0].identifier : "");
  const ruleDefinition = props.config.rules.find(
    (r) => r.identifier === defaultIdentifier,
  );
