  ...
  <VTransition>
    ...
    >
      ...
      <div class="flex flex-col flex-1 max-md:max-w-full">
        <div
          class="flex flex-col text-sm leading-5 text-slate-900 max-md:max-w-full"
        >
          <h5
          ...
          <p class="mt-2 max-md:max-w-full text-slate-500">
            <span v-if="specifyFormOwner"
              >Only you are seeing this notification, as owner of the
              form.</span
            >
            The following features are disabled on the published form:
          </p>
          <div
            class="text-slate-500 break-words whitespace-break-spaces"
            v-html="sanitizedCleaningContent"
          />
        </div>
        <div
          class="flex gap-0 self-start mt-2 text-xs font-medium leading-3 text-center"
        >
          <TrackClick
          ...
          >
            ...
            >
              ...
              <template v-else> Unlock all features </template>
            </UButton>
          </TrackClick>
          <UButton
...
const shouldShowWarning = computed(() => {
  ...
  if (
    props.useCookieDismissal &&
    dismissedCookie.value &&
    isWithinLastHour(dismissedCookie.value)
  ) {
    return false;
  }
...
// Sanitize cleaningContent before rendering — msg values come from server-side
// form.cleanings and could contain malicious HTML if that data is ever compromised
const sanitizedCleaningContent = computed(() =>
  $sanitize(cleaningContent.value),
);
...
const onUpgradeClick = () => {
  openSubscriptionModal({
    ...
    modal_description:
      "Some features are disabled on the published form. Upgrade your plan to unlock these features and much more. Gain full access to all advanced features.",
  });
};
