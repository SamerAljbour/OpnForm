<template>
  <UDropdownMenu :items="shareItems" :content="{ align: 'end' }" arrow>
    ...
    >
      <UTooltip text="Share">
        <UButton variant="outline" color="neutral" icon="i-heroicons-share" />
      </UTooltip>
    </TrackClick>
  </UDropdownMenu>
</template>
  ...
  computed: {
    shareItems() {
      return [
        [
          {
            label: "المشاركة عبر البريد الإلكتروني",
            icon: "i-heroicons-envelope",
            onSelect: () => this.shareViaEmail(),
          },
          {
            label: "المشاركة على تويتر",
            icon: "i-simple-icons-twitter",
            onSelect: () => this.shareOnTwitter(),
          },
          {
            label: "المشاركة على لينكد إن",
  ...
  methods: {
    shareViaEmail() {
      ...
      const subject = encodeURIComponent(
        `Check out this form: ${this.form.title}`,
      );
      const body = encodeURIComponent(
        `I'd like to share this form with you: ${this.shareUrl}`,
      );
