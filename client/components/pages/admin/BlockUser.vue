<template>
  ...
  >
    <div class="space-y-6 flex flex-col justify-between">
      ...
      >
        <template #description>
          <div v-html="sanitizedAlertContent" />
        </template>
      </UAlert>
    ...
    >
      <template #body>
        <UTable :columns="historyColumns" :data="blockingHistory" />
      </template>
    </UModal>
  </AdminCard>
</template>
...
const historyColumns = [
  {
    ...
    header: "Blocked At",
    cell: ({ row }) => {
      return row.original.blocked_at
        ? new Date(row.original.blocked_at).toLocaleString()
        : "";
    },
  },
  {
  ...
  },
  {
    ...
    header: "Unblocked At",
    cell: ({ row }) => {
      return row.original.unblocked_at
        ? new Date(row.original.unblocked_at).toLocaleString()
        : "";
    },
  },
  {
...
const alertContent = computed(() => {
  if (isBlocked.value) {
    const blockedBy =
      lastBlock.value?.blocked_by || "Automatically blocked by our AI";
    return {
...
// Sanitize the alert content before rendering — blockedBy and reason come from
// the database and could contain malicious HTML if an attacker poisoned those fields
const sanitizedAlertContent = computed(() =>
  $sanitize(alertContent.value.content),
);
