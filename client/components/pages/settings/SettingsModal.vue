  ...
  >
    <template #content>
      <div class="flex h-full flex-col sm:flex-row">
        <!-- Left Sidebar -->
        <aside
          class="flex flex-col border-b border-neutral-200 bg-neutral-50 sm:w-56 sm:shrink-0 sm:border-r sm:border-b-0"
        >
          <!-- Navigation Menu -->
...
watch(isOpen, (newValue) => {
  if (newValue) {
    ...
    nextTick(() => {
      const isValidPropTab =
        props.activeTab &&
        registeredPages.value.some((p) => p.id === props.activeTab);
      ...
      if (isValidPropTab) {
        ...
        const isCurrentTabValid = registeredPages.value.some(
          (p) => p.id === activeTabRef.value,
        );
...
watch(
  ...
  () => {
    ...
    if (
      props.activeTab &&
      props.activeTab !== activeTabRef.value &&
      registeredPages.value.some((p) => p.id === props.activeTab)
    ) {
      activeTabRef.value = props.activeTab;
      return;
...
function registerModalPage(id, label, icon) {
  const existingIndex = registeredPages.value.findIndex(
    (page) => page.id === id,
  );
  if (existingIndex !== -1) {
