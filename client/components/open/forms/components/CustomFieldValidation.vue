  ...
  <div class="py-2 px-4">
    ...
    <div class="mt-4">
      <p class="text-xs font-medium text-gray-600 mb-2">
        متى يجب أن يكون هذا الحقل صحيحًا؟
      </p>
      <p class="text-neutral-500 text-xs mb-3">
        ضع <span class="font-semibold">قواعد تجعل هذا الحقل صحيحًا</span>. إذا
        لم تتم قيودها، يظهر الحقل خطأ.
      </p>
      <div
    ...
    <div>
      <p class="text-xs font-medium text-gray-600 mb-2">
        ما هي الرسالة التي يجب عرضها إذا فشل التحقق؟
      </p>
      <p class="text-neutral-500 text-xs mb-3">
        تظهر هذه الرسالة عندما
        <span class="font-semibold">لم يتم استيفاء قواعد التحقق. </span>.
      </p>
      <div
        class="p-3 border border-gray-200 rounded-lg bg-gray-50/50 hover:bg-gray-50 transition-colors"
      >
    ...
    <!-- <UAlert
      ...
      size="sm"
      class="mt-4"
      description="تتوفر التحقق المخصص فقط في الخطط المدفوعة."
    /> -->
...
export default {
  ...
  computed: {
    conditionsCount() {
      if (
        ...
        this.validation.conditions === undefined
      )
        return 0;
  ...
  methods: {
    ...
    openHelpArticle() {
      this.crisp.openHelpdeskArticle(
        "how-to-add-custom-validation-to-a-field-elzuxy",
      );
    },
    clearAll() {
      this.validation.conditions = null;
      ...
      if (this.copyFrom) {
        ...
        if (sourceField && sourceField.validation?.error_conditions) {
          ...
          if (sourceField.validation.error_message) {
            this.field.validation.error_message =
              sourceField.validation.error_message;
          }
        }
      }
      ...
      if (this.copyTo.length) {
        this.copyTo.forEach((fieldId) => {
          ...
          if (targetField) {
            ...
            targetField.validation.error_conditions = clonedeep(
              this.validation,
            );
            if (this.field.validation?.error_message) {
              targetField.validation.error_message =
                this.field.validation.error_message;
            }
          }
        });
