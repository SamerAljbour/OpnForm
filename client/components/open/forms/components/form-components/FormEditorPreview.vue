<template>
  ...
  <div
    ...
    :class="{
      ...
      'bg-neutral-50 dark:bg-notion-dark-light hidden md:flex flex-grow p-4 flex-col items-center shadow-inner':
        !isExpanded,
    }"
  >
    <div
    ...
    >
      <div
        class="w-full bg-white dark:bg-neutral-950 border-b border-neutral-300 dark:border-blue-900 dark:border-neutral-700 rounded-t-lg p-1.5 pl-4 pr-1.5 flex items-center gap-x-1.5"
      >
        ...
        <UButton
          v-if="
            ...
            (form &&
              ...
              workingFormStore.formPageIndex !== 0)
          "
      ...
      <div
        class="flex-grow overflow-y-auto relative flex flex-col transform-gpu"
      >
        ...
        <VTransition name="fade">
          ...
          >
            <UTooltip text="Add block" :kbds="['meta', 'B']" arrow>
              <UButton
...
const formMode = computed(() =>
  isExpanded.value ? FormMode.TEST : FormMode.PREVIEW,
);
...
function getAbsoluteIndexFromVisiblePage(visiblePageIndex) {
  const struct = workingFormStore.structureService;
  if (!struct || typeof struct.getPageFields !== "function")
...
watch(
  ...
  (newIndex) => {
    try {
      ...
      if (
        !isValidIndex(absoluteIndex) ||
        workingFormStore.selectedFieldIndex === absoluteIndex
      )
        return;
      ...
      if (
        field &&
        struct &&
        typeof struct.isFieldHidden === "function" &&
        struct.isFieldHidden(field)
      ) {
        return;
      }
...
watch(
  () => workingFormStore.selectedFieldIndex,
  (newIndex) => {
    try {
...
function handleAddBlock() {
  try {
    ...
    const absoluteIndex = getAbsoluteIndexFromVisiblePage(
      currentSlideIndex.value,
    );
