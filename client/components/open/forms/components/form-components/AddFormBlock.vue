<template>
  <div class="pb-16" @keydown="handleKeydown" tabindex="-1">
    <div class="p-2 border-b border-neutral-300 sticky top-0 z-10 bg-white">
      <div class="flex items-center">
        <UButton
          ...
          @click="closeSidebar"
        />
        <div class="font-medium inline ml-2 flex-grow truncate py-2 px-4">
          إضافة حقل
        </div>
        <!-- <AiFieldGenerator class="py-2 px-4" /> -->
      </div>
    </div>
    ...
    <div class="py-2 px-4">
      <p class="text-neutral-500 text-xs font-medium my-2">عناصر الادخال</p>
      <VueDraggable
      ...
      >
        <template #default>
          ...
          >
            لا توجد عناصر إدخال تطابق بحثك.
          </p>
        </template>
      </VueDraggable>
    </div>
    <div class="px-4 border-t mb-4">
      <p class="text-neutral-500 text-xs font-medium my-2">عناصر التصميم</p>
      <VueDraggable
      ...
      >
        <template #default>
          <div
            ...
            role="button"
            :tabindex="
              selectedIndex === filteredInputBlocks.length + index ? 0 : -1
            "
            @click.prevent="addBlock(element.name)"
            @keydown.enter.prevent="addBlock(element.name)"
          >
...
const formStyle = computed(
  () => workingFormStore.content?.presentation_style || "classic",
);
...
const scrollToSelected = () => {
  nextTick(() => {
    if (selectedIndex.value >= 0) {
      ...
      const element =
        blockRefsMap.get(selectedIndex.value) ||
...
const handleKeydown = (event) => {
  ...
  switch (event.key) {
    case "ArrowDown":
      ...
      selectedIndex.value =
        selectedIndex.value < totalBlocks - 1 ? selectedIndex.value + 1 : 0;
      scrollToSelected();
      break;
    ...
    case "ArrowUp":
      ...
      selectedIndex.value =
        selectedIndex.value > 0 ? selectedIndex.value - 1 : totalBlocks - 1;
      scrollToSelected();
      break;
    ...
    case "Enter":
      if (
        ...
        selectedIndex.value < totalBlocks.length
      ) {
