<template>
  <form v-if="form" class="@container pb-20" @submit.prevent="">
    <!-- Classic cover/logo rendering -->
    <div v-if="showBrandingMedia" class="mb-2">
      <div v-if="form.cover_picture">
        <div
          id="cover-picture"
          class="h-56 w-full overflow-hidden pointer-events-none"
        >
          <BlockMediaLayout
            :image="coverMedia"
      ...
      <div
        ...
        :class="[
          ...
          form.width === 'centered'
            ? isPreviewMode
              ? 'max-w-lg'
              : '@3xl:w-3/5 @5xl:w-1/2 @3xl:max-w-2xl'
            : '',
      ...
      >
        <img
          ...
          :alt="
            form.seo_meta?.site_name
              ? `${form.seo_meta.site_name} logo`
              : 'Form logo'
          "
          :class="{
    ...
    >
      ...
      <transition name="fade" mode="out-in">
        ...
        <div
          ...
          key="submitted"
          class="px-2 w-full flex flex-col items-center justify-center text-center"
        >
          <slot name="after-submit" :submittedData="submittedData" />
        </div>

        <div
          v-else-if="shouldDisplayForm"
          :key="formPageIndex"
        ...
        >
          <VueDraggable
            ...
            :class="[
              draggingNewBlock
                ? 'rounded-md bg-blue-50 dark:bg-neutral-800'
                : '',
            ]"
      ...
      <!-- Captcha -->
      <CaptchaWrapper
        v-if="form.use_captcha && !isSubmitted"
        :form-manager="formManager"
      />
      ...
      >
        ...
        <div v-if="hasPaymentBlock" class="mt-6 flex justify-center w-full">
          <p
            class="text-xs text-neutral-400 dark:text-neutral-500 flex text-center max-w-md"
          >
      ...
      <!-- Cleanings slot -->
      <div
        class="fixed bottom-4 left-4 max-w-full z-10"
        v-if="$slots.cleanings"
      >
        <div class="max-w-lg">
          <slot name="cleanings" />
        </div>
    ...
    <!-- Branding button -->
    <div
      v-if="!form.no_branding && showBranding"
      class="fixed z-10 bottom-4 right-4"
    >
...
const hasPaymentBlock = computed(
  () => structure.value?.currentPageHasPaymentBlock?.value ?? false,
);

const currentFields = computed(
  () => structure.value?.getPageFields?.(state.value.currentPage) ?? [],
);
...
const currentFieldsPageBreak = computed(
  () => structure.value?.currentPageBreak?.value ?? null,
);
const previousFieldsPageBreak = computed(
  () => structure.value?.previousPageBreak?.value ?? null,
);
...
const getAbsoluteIndex = (relativeIndex) => {
  return structure.value.getTargetDropIndex(
    relativeIndex,
...
const shouldDisplayForm = computed(() => {
  const showAdminControls =
    !!props.formManager?.strategy?.value?.admin?.showAdminControls;
  return (
    (!isSubmitted.value &&
      !form.value?.is_closed &&
      !form.value?.max_number_of_submissions_reached &&
      !slots.password) ||
    showAdminControls
  );
});
...
const isPreviewMode = computed(
  () => props.formManager?.mode?.value === FormMode.PREVIEW,
);
...
const showBrandingMedia = computed(() => {
  ...
  return !!(
    form.value &&
    (form.value.logo_picture || form.value.cover_picture)
  );
});
...
const showBranding = computed(
  () => props.formManager?.strategy?.value?.display?.showBranding ?? true,
);
</script>
