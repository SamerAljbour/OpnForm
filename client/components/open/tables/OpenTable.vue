<template>
  ...
  >
    <div ref="topBar" class="flex items-center gap-2 p-2 overflow-x-auto">
      ...
      <TableColumnManager class="ml-auto" :table-state="tableState" />
      ...
      <UTooltip arrow :text="isExpanded ? 'خروج من ملء الشاشة' : 'ملء الشاشة'">
        <UButton
          size="sm"
          color="neutral"
          variant="ghost"
    ...
    >
      ...
      <template #actions-cell="{ row }">
        <div
          ...
          :style="{ width: `var(--col-actions-size, auto)` }"
        >
          <RecordOperations
            :form="form"
...
const {
  ...
  columnWrapping,
  columnOrder,
  handleColumnResize: handleColumnResizeState,
} = tableState;

const tableColumns = computed(() => {
  return allColumns.value.filter(
    (column) => !["actions", "select"].includes(column.id),
  );
});
...
const onDeleteMultiClick = () => {
  alert.confirm(
    `Do you really want to delete selected ${selectedIds.value.length} record${selectedIds.value.length > 1 ? "s" : ""}?`,
    deleteMultiRecord,
  );
};
const deleteMultiRecord = () => {
  deleteMultiSubmissionsMutation
    .mutateAsync({
