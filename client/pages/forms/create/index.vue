<template>
  <div class="flex flex-wrap flex-col flex-grow">
    <div key="2" class="w-full flex flex-grow flex-col">
...
onBeforeRouteLeave((to, from, next) => {
  if (isDirty()) {
    if (
      window.confirm(
        "قد لا يتم حفظ التغييرات التي أجريتها. هل أنت متأكد أنك تريد المغادرة؟",
      )
    ) {
...
onMounted(() => {
  ...
  form.value = initForm(
    { workspace_id: workspace.value?.id, no_branding: workspace.value?.is_pro },
    true,
  );
