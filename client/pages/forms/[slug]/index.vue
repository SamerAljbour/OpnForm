<template>
  <div id="public-form" class="flex flex-col min-h-screen">
...
onMounted(() => {
  ...
  if (form.value) {
    ...
    if (import.meta.client) {
      const allowSelfHosted = !!useFeatureFlag(
        "custom_code.enable_self_hosted",
        false,
      );
      const isSelfHosted = !!useFeatureFlag("self_hosted", false);
      const isCustomDomain = customDomainUsed();

      const canExecuteCustomCode =
        isCustomDomain || (isSelfHosted && allowSelfHosted);
...
const trackFormView = () => {
  if (
    import.meta.client &&
    !form.value?.is_password_protected &&
    !hasViewedForm
  ) {
    hasViewedForm = true;
    nextTick(() => {
