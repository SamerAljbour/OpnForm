<template>
  <div class="p-4">
    <div class="w-full max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-2">
      <div
        v-for="(stat, index) in [
          { label: 'المشاهدات', value: totalViews, placeholder: '123' },
          { label: 'الإجابات', value: totalSubmissions, placeholder: '123' },
          {
            label: 'اكتمال',
      ...
      >
        ...
        <VTransition name="fade">
          <USkeleton v-if="isLoading" class="h-7 w-16" />
          <span v-else-if="form.is_pro" class="font-medium text-xl">
            {{ stat.value }}
          </span>
          <span v-else class="blur-[3px] pointer-events-none">
            {{ stat.placeholder }}
          </span>
        </VTransition>
      </div>
...
const completionRate = computed(() =>
  Math.min(100, statsData.value?.completion_rate ?? 0),
);
const averageDuration = computed(
  () => statsData.value?.average_duration ?? "-",
);
</script>
